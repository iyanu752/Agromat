...
export default function LoginPage() {
  ...
  // Clear admin passkey when switching away from admin tab
  useEffect(() => {
    if (activeTab !== "admin") {
      setadminPassKey("");
    }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await loginUser(email, password, userType, adminPassKey);
  ...
  const renderLoginForm = (role: string, label: string) => (
    <form className="mt-4 space-y-4" onSubmit={handleLogin}>
      ...
      {/* Admin Passkey field - only show for admin login */}
      {role === "admin" && (
        <div className="space-y-2">
